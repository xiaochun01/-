...
class CheckUtils:
    ...
    def __key_check(self,actual_result,check_data ):
        ...
        if self.fail_result in tmp_result:
            ...
            logger.info('测试实际结果：%s==测试期望结果：%s，检测通过'%(json.dumps(actual_result,ensure_ascii=False),check_data))
    ...
    def run_check(self,check_type,chech_data):
        logger.info('根据%s进行断言，检查是否满足期望结果：%s'%(check_type,chech_data,))
        if check_type=='none' or chech_data=='':
            return self.check_rules[check_type]()
        else:
            return self.check_rules[check_type](chech_data)


    # def header_key(self,check_data):
    #     key_list = check_data.split(',')
    #     tmp_result = []
    #     for key in  key_list:
